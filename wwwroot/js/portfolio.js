...
(function () {
    ...
    async function apiErrorMessage(res, fallback) {
        try {
            ...
            if (Array.isArray(body) && body.length) return body.join("; ");
    ...
    async function apiSend(method, path, body) {
        ...
        if (!res.ok) throw new Error(await apiErrorMessage(res, method + " " + path + " failed (" + res.status + ")"));
        return res.status === 204 ? null : res.json();
    }
        ...
        safe.projects = (Array.isArray(safe.projects) ? safe.projects : [])
            .map(function (project) {
                ...
                if (typeof normalized.intakeDetails === "string") {
                    try {
                        normalized.intakeDetails = normalized.intakeDetails ? JSON.parse(normalized.intakeDetails) : null;
                    } catch (error) {
    ...
    async function loadPortfolio() {
        try {
            return normalize(await apiGet("/portfolio"));
        } catch (error) {
            console.error(error);
            Argo.toast("Could not reach the Argo server \u2014 showing an empty board: " + error.message);
            return clone(cleanSeed);
        }
    }
    ...
    function statusClass(value) {
        return "status-" + String(value || "Not Started")
            .replace(/ /g, "-");
    }
    ...
    function ownerMatches(projectOwner, teamMember) {
        var owner = String(projectOwner || "")
            .trim()
            .toLowerCase();
        var member = String(teamMember || "")
            .trim()
            .toLowerCase();
    ...
    function displayText(value, fallback) {
        return Argo.escapeHtml(value || fallback || "Not entered");
    }
    ...
    function renderSummary() {
        ...
        document.getElementById("summary")
            .innerHTML = statuses.map(function (status) {
                var count = data.projects.filter(function (project) {
    ...
    function renderTeamChart() {
        ...
        var unassigned = data.projects.filter(function (project) {
            return project.status !== "Done" && String(project.owner)
                .toLowerCase() === "unassigned";
        })
            .length;
        var rows = TEAM_MEMBERS.map(function (member) {
            var projects = data.projects.filter(function (project) {
                return ownerMatches(project.owner, member);
            });
            var grouped = {};
            buckets.forEach(function (bucket) {
                grouped[bucket] = projects.filter(function (project) {
                    return chartBucket(project) === bucket;
                });
            });
            return {
                member: member,
        ...
        var legend = (unassigned ? '<span class="triage-count"><strong>' + unassigned + '</strong> awaiting assignment</span>' : "") + buckets.map(function (bucket) {
            return '<span class="legend-item"><span class="legend-dot chart-' + healthClass(bucket) + '"></span>' + Argo.escapeHtml(bucket) + '</span>';
        })
            .join("");
        var bars = rows.map(function (row) {
            var segments = buckets.map(function (bucket) {
                ...
                var names = matches.map(function (project) {
                    return project.name;
                })
                    .join("; ");
            ...
            .join("");
        document.getElementById("team-chart")
            .innerHTML = '<div class="chart-head"><div><h2>Team workload &amp; project health</h2><p>Each project appears once. At Risk and Blocked take precedence so attention items remain visible.</p></div><div class="chart-legend">' + legend + '</div></div><div class="owner-bars">' + bars + '</div>';
    }

    function renderBoard() {
        var lanes = [{
            status: "Waiting",
            hint: "Submitted, triaged, or queued for work"
        },
        {
            status: "In Progress",
        ...
        {
            status: "Done",
            hint: "Deliverables accepted"
        }
        ];
        document.getElementById("board")
            .innerHTML = lanes.map(function (lane) {
                var projects = data.projects.filter(function (project) {
                    return project.status === lane.status;
                });
                if (lane.status === "Waiting") projects.sort(function (a, b) {
                    ...
                    return difference || String(a.name)
    ...
    function intakeValue(value) {
        return Array.isArray(value) ? value.join(", ") : String(value || "");
    }

    function intakeRow(label, value) {
        var shown = intakeValue(value);
        if (!shown) return "";
        return '<div class="intake-row"><div class="intake-label">' + Argo.escapeHtml(label) + '</div><div class="intake-value">' + Argo.escapeHtml(shown) + '</div></div>';
    }

    function intakeSection(title, rows, size) {
        var content = rows.map(function (row) {
            return intakeRow(row[0], row[1]);
        })
            .join("");
        if (!content) return "";
        return '<section class="intake-section ' + (size || "") + '"><h4>' + Argo.escapeHtml(title) + '</h4><div class="intake-rows">' + content + '</div></section>';
    }

    function renderIntakePanel(project) {
        var d = project.intakeDetails;
        if (!d) return "";
        var submitted = project.submittedAt ? new Date(project.submittedAt)
        ...
        var html = "";
        html += intakeSection("Request and ownership", [
            ["Request ID", project.sourceRequestId || d.requestId],
            ["Submitted", submitted],
            ["Request type", d.requestType],
            ["Requester", d.requesterName],
            ["Department / team", d.department],
            ["Contact", d.requesterContact],
            ["Business sponsor", d.businessSponsor],
            ["Business owner", d.businessOwner],
            ["Additional stakeholders", d.additionalStakeholders]
        ], "");
        html += intakeSection("Business need", [
            ["Request description", d.requestDescription],
            ["Problem / opportunity", d.businessProblem],
            ["Desired outcome", d.desiredOutcome],
            ["Success measures", d.successMeasures],
            ["Affected groups", d.affectedGroups],
            ["Current workaround", d.currentProcess]
        ], "wide");
        html += intakeSection("Impact and timing", [
            ["Business impact", d.businessImpact],
            ["Reach", d.impactScope],
            ["Users affected", d.usersAffected],
            ["Client impact", d.clientImpact],
            ["Clients / logos", d.clientNames],
            ["Expected benefits", d.expectedBenefits],
            ["If nothing changes", d.noActionImpact],
            ["Desired date", d.desiredDate],
            ["Date type", d.dateType],
            ["Why the date matters", d.dateReason]
        ], "");
        html += intakeSection("Scope and dependencies", [
            ["In scope", d.inScope],
            ["Out of scope", d.outOfScope],
            ["Dependencies", d.dependencies],
            ["Strategic alignment", d.strategicAlignment]
        ], "");
        html += intakeSection("Systems and data", [
            ["Systems / tools", d.systemsInvolved],
            ["Data sources", d.dataSources],
            ["Sensitive data", d.sensitiveData],
            ["Data concern", d.sensitiveDetails],
            ["Access needed", d.accessNeeded],
            ["Technical owners", d.technicalOwners],
            ["Vendors / partners", d.vendors],
            ["Supporting materials", d.supportingMaterials]
        ], "wide");
        if (d.requestType === "Reporting or dashboard request") html += intakeSection("Reporting details", [
            ["Reports", d.reportNames],
            ["Frequency", d.reportFrequency],
            ["Delivery time", d.deliveryTime],
            ["Recipients", d.reportRecipients],
            ["Output format", d.outputFormat],
            ["Samples available", d.samplesAvailable],
            ["Sample references", d.sampleReferences],
            ["Manual steps", d.manualSteps]
        ], "full");
        if (d.requestType === "Data integration/source request") html += intakeSection("Data integration details", [
            ["Source system", d.sourceSystem],
            ["Target system", d.targetSystem],
    ...
    function renderDetail() {
        ...
        if (!project) {
            detail.innerHTML = '<div class="empty">Add a project to begin.</div>';
            return;
        }
        var work = data.workItems.filter(function (item) {
            return item.projectId === project.id;
        });
        var raid = data.raidItems.filter(function (item) {
            return item.projectId === project.id;
        });
        var completed = work.filter(function (item) {
            return item.status === "Done";
        })
            .length;
        var progress = work.length ? Math.round(completed / work.length * 100) : 0;
        var packages = work.map(renderWorkPackage)
            .join("") || '<div class="empty">No work packages yet.</div>';
        var raidCards = raid.map(function (item) {
            return '<button type="button" class="raid-card" data-action="edit-raid" data-id="' + Argo.escapeHtml(item.id) + '"><span class="badge">' + Argo.escapeHtml(item.type) + '</span><span class="raid-copy"><strong>' + Argo.escapeHtml(item.description) + '</strong><span class="raid-meta"><span>' + Argo.escapeHtml(item.owner) + '</span><span>' + Argo.escapeHtml(dateLabel(item.dueDate)) + '</span></span></span><span>✎</span></button>';
        })
        ...
        if (editButton) {
            ...
            actionGroup.className = "detail-actions";
            editButton.replaceWith(actionGroup);
            actionGroup.appendChild(editButton);
            actionGroup.insertAdjacentHTML("beforeend", '<button class="btn danger" type="button" data-action="delete-project" data-id="' + Argo.escapeHtml(project.id) + '">Delete project</button>');
        }
        if (project.intakeDetails) detail.querySelector(".detail-head")
            .insertAdjacentHTML("afterend", renderIntakePanel(project));
    }
    ...
    function syncPriorityField() {
        document.getElementById("project-priority-field")
            .hidden = document.getElementById("project-status")
                .value !== "Waiting";
    }
    ...
    function openProject(id) {
        ...
        fillForm(document.getElementById("project-form"), record);
        syncPriorityField();
        document.getElementById("project-dialog-title")
            .textContent = id ? "Project " + id : "New project";
    ...
    function openWork(id) {
        ...
        fillForm(document.getElementById("work-form"), record);
        document.getElementById("work-dialog")
            .showModal();
    }

    function openActivity(workItemId, id) {
        var project = selected();
        ...
        fillForm(document.getElementById("activity-form"), record);
    ...
    async function deleteProject(id) {
        ...
        if (!project) return;
        var linkedWorkIds = data.workItems.filter(function (item) {
        ...
        var activityCount = data.activities.filter(function (item) {
            return item.projectId === id || linkedWorkIds.indexOf(item.workItemId) >= 0;
        })
            .length;
        var raidCount = data.raidItems.filter(function (item) {
            return item.projectId === id;
        })
            .length;
        ...
        try {
            ...
            await refresh();
            linkedWorkIds.forEach(function (workId) {
                delete expandedWorkIds[workId];
            });
            selectedId = data.projects[0] ? data.projects[0].id : "";
            render();
            Argo.toast("Project and linked records deleted");
        } catch (error) {
            console.error(error);
            Argo.toast("Could not delete the project \u2014 " + error.message);
        }
    }
    ...
    document.addEventListener("click", function (event) {
        ...
        if (action === "toggle-work") {
            expandedWorkIds[id] = !expandedWorkIds[id];
            renderDetail();
        }
        if (action === "add-activity") openActivity(button.dataset.workId);
        if (action === "edit-activity") {
            var activity = data.activities.find(function (item) {
                return item.id === id;
            });
            if (activity) openActivity(activity.workItemId, id);
        }
        if (action === "add-raid") openRaid();
        if (action === "edit-raid") openRaid(id);
        if (button.dataset.close) document.getElementById(button.dataset.close)
            .close();
    });
    ...
    document.getElementById("project-status")
        .addEventListener("change", syncPriorityField);
    document.getElementById("board")
        .addEventListener("change", function (event) {
            if (event.target.dataset.action === "sort-waiting") {
                waitingSort = event.target.value;
                renderBoard();
            }
        });
        ...
        .addEventListener("submit", async function (event) {
            ...
            try {
                ...
                document.getElementById("project-dialog")
                    .close();
                render();
                Argo.toast("Project saved");
            } catch (error) {
                console.error(error);
                Argo.toast("Could not save the project \u2014 " + error.message);
            }
        });
        ...
        .addEventListener("submit", async function (event) {
            ...
            try {
                ...
                document.getElementById("work-dialog")
                    .close();
                render();
                Argo.toast("Work package saved");
            } catch (error) {
                console.error(error);
                Argo.toast("Could not save the work package \u2014 " + error.message);
            }
        });
        ...
        .addEventListener("submit", async function (event) {
            ...
            try {
                ...
                document.getElementById("activity-dialog")
                    .close();
                render();
                Argo.toast("Activity saved");
            } catch (error) {
                console.error(error);
                Argo.toast("Could not save the activity \u2014 " + error.message);
            }
        });
    document.getElementById("raid-form")
        .addEventListener("submit", async function (event) {
            event.preventDefault();
            var record = formObject(event.currentTarget);
            try {
    ...
    async function init() {
        try {
            ...
            render();
            Argo.toast("Could not reach the Argo server: " + error.message);
        }
    }
    init();
})();